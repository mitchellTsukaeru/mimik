      ...
      {results.map((r, i) => (
        ...
        >
          <div
            ...
            style={
              i === selected
                ? { background: 'rgba(253,230,138,0.15)', border: '1px solid rgba(253,230,138,0.2)', color: 'var(--color-gold)' }
                : { background: 'var(--color-secondary)', border: '1px solid var(--color-border)', color: 'var(--color-muted-foreground)' }
            }
          >
            {r.favicon ? (
              <img
          ...
          <div className="flex-1 min-w-0">
            ...
            >
              {r.guide.stepIds.length} step{r.guide.stepIds.length !== 1 ? 's' : ''} · {formatDateShort(r.guide.updatedAt)}
            </p>
          </div>
        </div>
      ))}
